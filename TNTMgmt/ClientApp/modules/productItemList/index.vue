<template>
  <div>
    <title-bar :langTitle="$t('screen.productItem')"></title-bar>
    <div class="content">
      <div class="panel panel-flat">
        <div class="panel-heading">
          <h5 class="panel-title">{{$t('screen.productItem')}}</h5>
            ...
            <el-dropdown @command="handleShowHide" trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <template v-for="(item, props, index) in headers">
                  <el-dropdown-item :key="index" :command="{props: props, item: item}">{{$t(`productItem.${props}`)}}<i v-show="item" class="el-icon-check mt-10 ml-10" style="float:right;"></i></el-dropdown-item>
                </template>
              </el-dropdown-menu>
            </el-dropdown>
            <router-link class="btn btn-primary fontDefault" to="/product/item/edit" :disabled="!disabled.edit">
              {{$t('createNew')}}
            </router-link>
          </div>
        </div>
            ...
            <div class="row">
              ...
              <div class="col-md-4">
                <div class="form-group has-feedback">
                  <input v-model="searchRequest.description" class="form-control" v-on:keyup.enter="search()" :placeholder="$t('productItem.searchDes')">
                ...
                <el-dropdown @command="handleChoosenfilter" placement="right-start" size="medium" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown" style="width:200px;padding-left:10px">
                    <template v-for="(item, index) in remainFilters">
                      <el-dropdown-item :command="item">{{$t('productItem'+'.'+item.field) }}</el-dropdown-item>
                    </template>
                  </el-dropdown-menu>
                </el-dropdown>
                    ...
                    <span tabindex="0" class="label bg-grey-300 pointer" style="padding: 6px; margin:0px 5px 0px 10px; font-size:13px">
                      <span @click="handleChoosenfilter(item)">
                        <span v-if="item.searchType == 'checkbox' && item.field == 'status'">
                          {{$t('productItem.' + item.field)}}:
                          <span v-for="(item1, index1) in item.data">
                            <span v-if="index1 < 2">{{ $t('productItem.' + item1.id)}}<span v-show="index1 == 0 && item.data.length > 1"> , </span></span>
                        ...
                        <span v-if="item.searchType == 'text'">
                          {{$t('productItem'+'.'+item.field)+ ': ' +item.filterText }}
                        </span>
                        <span v-if="item.searchType == 'date'">
                          {{$t('productItem'+'.'+item.field)}}: {{$moment(item.date[0]).format('DD/MM/YYYY')}} {{$t('goto')}} {{$moment(item.date[1]).format('DD/MM/YYYY')}}
                        </span>
                      </span>
                  ...
                  <div class="panel-body">
                    <div v-if="advanceFilter.showData.searchType == 'checkbox'">
                      ...
                      <div v-for="(item, index) in filterData">
                        ...
                        <label :for="item.id" class="pointer label-padding break-word">{{(item.name != item.id) ? item.name : $t('productItem.' + item.name)}}</label>
                      </div>
                    </div>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class='table-cont' id='table-cont' style="padding-top: 0px;">
              <table class="table">
                  ...
                  <tr>
                    ...
                    <template v-for="(item, props, index) in headers">
                      <template v-if="index==1">
                        <th :key="index" v-show="item">{{$t(`productItem.${props}`)}}</th>
                        <th v-show="item"></th>
                      </template>
                      <template v-else>
                        <th :key="index" v-show="item">{{$t(`productItem.${props}`)}}</th>
                ...
                <tbody>
                  <tr v-for="(item, i) in productItem.data" :key="i" :value="i">
                    ...
                    <td v-show="headers['name']">
                      <router-link :to="`/product/item/edit/${item.id}`" :disabled="!disabled.edit">
                        <span class="break-word blue--text">{{item.name || ''}}</span>
                      </router-link>
                    </td>
                    <td v-show="headers['name']">
                      <div class="icons-list">
                        <el-tooltip :content="$t('productItem.updateProductItem')" placement="top" effect="dark">
                          <router-link :to="`/product/item/edit/${item.id}`" :disabled="!disabled.edit">
                    ...
                    <td v-show="headers['productCategoryId']">
                      <span>{{item.productCategoryData}}</span>
                    </td>
                    <td v-show="headers['status']">
                      <div class="pb-5">
                        <el-tooltip placement="top" effect="dark">
                          <span slot="content">{{$t('productItem.status')}}</span>
                        ...
                        <el-dropdown class="fontDefault" trigger="click" @command="handleStatusClick">
                          <span class="el-dropdown-link changeOnLeads">
                            {{item.status ? $t('productItem.' + item.status) : $t('notAvailableStatus')}}
                          </span>
                          <el-dropdown-menu slot="dropdown" class="el-popper-default">
                            <el-dropdown-item key="itemNull"
                                              value="null"
                                              :command="{statusId: null, productItem: item}"
                            ...
                            <el-dropdown-item v-for="(statusItem, statusKey) in commonData.statusProduct"
                                              ...
                                              :command="{statusId: statusItem.value, productItem: item}"
                                              :disabled="!disabled.edit">
                              {{$t('productItem.' + statusItem.value)}}
                            </el-dropdown-item>
                          </el-dropdown-menu>
                        </el-dropdown>
                      </div>
                    </td>
                    <td v-show="headers['desc']">
                      <span>{{item.desc}}</span>
                    </td>
          ...
          <div class="total-list">
            <div v-if="!productItem.total">{{$t('noData')}}</div>
            <div v-else>{{$t('have')}} {{productItem.total}} {{$t('row')}}</div>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        productItem: "$_productItemList/productItem",
        remainFilters: '$_productItemList/remainFilters',
        filterData: '$_productItemList/filterData',
        currentUser: 'common/currentUser'
      }),
      ...mapState({
        searchRequest: state => state.$_productItemList.searchRequest,
        searchRequestCategory: state => state.$_productItemList.searchRequestCategory,
        headers: state => state.$_productItemList.headers,
        advanceFilter: state => state.$_productItemList.advanceFilter,
      }),
      disabled() {
    ...
    watch: {
      "isCheckAll": function (val) {
        var _this = this;
        _this.ids = val ? _.map(_this.productItem.data, (o) => { return o.id }) : [];
      },
      "desFilter": _.debounce(async function (val) {
        var _this = this;
        if (_this.advanceFilter.showData.field == 'productCategoryId') {
          _this.searchRequestCategory.description = val;
          await _this.$store.dispatch('$_productItemList/getProductCategory');
        } else {
          _this.$store.dispatch('$_productItemList/updateFilterCheckbox', { data: val, type: "desFilter" });
        }
      }, 200),
    },
    async created() {
      var _this = this;
      const STORE_KEY = "$_productItemList";
      if (!(STORE_KEY in this.$store._modules.root._children)) {
        _this.$store.registerModule(STORE_KEY, store);
      }
      await _this.$store.dispatch("common/setTitle", _this.$t("screen.productItem"));
      await _this.$store.dispatch("$_productItemList/getProductCategory");
      if (_this.productItem && !_this.productItem.data) await _this.$store.dispatch("$_productItemList/getProductItem");
    },
    methods: {
      filter: _.debounce(async function () {
        var _this = this;
        await _this.$store.dispatch("$_productItemList/getProductItem");
      }, 500),
      ...
      async handleShowHide(command) {
        var _this = this;
        await _this.$store.dispatch("$_productItemList/showHideCols", command);
      },
      deleteData: _.debounce(async function (data) {
        var _this = this;
        await _this.$confirm(_this.$t('productItem.deleteProductItem'), _this.$t('productItem.deleteTitle'), {
          ...
          try {
            await _this.$store.dispatch(
              `$_productItemList/deleteProductItem`,
      ...
      async handleChoosenfilter(command) {
        ...
        _this.desFilter = '';
        if (command.field == 'productCategoryId') {
          await _this.$store.dispatch('$_productItemList/getProductCategory');
        }
      ...
      async applyFilter() {
        ...
        if ((request.searchType == "checkbox" && request.data.length > 0) || (request.searchType == "text" && request.filterText) || (request.searchType == "date" && request.date)) {
          await _this.$store.dispatch('$_productItemList/updateFilterProdItem', request)
        }
      },
      async cancelFilter(field) {
        var _this = this;
        _this.desFilter = '';
        await _this.$store.dispatch('$_productItemList/updateFilterProdItem', { field: field, isDelete: true })
      },
        ...
        await _this.$validator.validateAll().then(async result => {
          if (result) {
            try {
              ...
              let response = await _this.$store.dispatch("$_productItemList/updateProductItem", formData);
              new PNotify({
                title: '',
      ...
      handleStatusClick(command) {
        var _this = this;
        if (command.productItem.status != command.statusId) {
          command.productItem.status = command.statusId;
          var formData = command.productItem;
          _this.validateBeforeSubmit(formData);
        }
      },
      search: _.debounce(async function () {
        await this.$store.dispatch("$_productItemList/getProductItem");
      }, 500),
    }
  };
</script>
